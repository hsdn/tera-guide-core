...
class Voice {
	...
	init() {
		...
		return new Promise((resolve, reject) => {
			this.__ps.start().then(() => this.__ps.invoke().then(() => resolve()).catch(error => { 
	 ...
	 * @memberof Voice
	 */
	speak(text, rate = 1, voice = false, volume = 100) {
		if (rate)
			this.__ps.addCommand(`$speak.Rate = ${parseInt(rate)}`);

		if (voice)
			this.__ps.addCommand(`$speak.SelectVoice("${voice}")`);

		if (volume)
			this.__ps.addCommand(`$speak.Volume = ${parseInt(volume)}`);
