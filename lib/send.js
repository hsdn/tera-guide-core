...
class Send {
	...
	message(message) {
		...
		if (this.mod.settings.stream)
			return this.mod.command.message(this.mod.settings.cc + this.functions.escapeHtml(message));

		let sending_event = {
			"message": this.mod.settings.cc + this.functions.escapeHtml(message)
		};

		if (this.params.chat_name)
			Object.assign(sending_event, { "name": this.params.chat_name });
	...
	notification(message) {
		// If streamer mode is enabled send message to the proxy-channel
		if (this.mod.settings.stream)
			return this.mod.command.message(`${clb}[Notice] ${this.mod.settings.cc}${this.functions.escapeHtml(message)}`);

		let sending_event = {
			"message": this.functions.escapeHtml(message)
		};
	...
	alert(message, cc, spc) {
		// If streamer mode is enabled send message to the proxy-channel
		if (this.mod.settings.stream)
			return this.mod.command.message(`${cc}[Alert] ${this.mod.settings.cc}${this.functions.escapeHtml(message)}`);
	...
	dungeonEvent(message, spcc, type) {
		// If streamer mode is enabled send message to the proxy-channel
		if (this.mod.settings.stream)
			return this.mod.command.message(spcc + this.functions.escapeHtml(message));
		...
		this.mod.toClient("S_DUNGEON_EVENT_MESSAGE", 2, {
			...
			"chat": 0,
			"channel": 27,
			"message": spcc + this.functions.escapeHtml(message)
		});
	}
	...
	// Play voice message
	speech(message) {
		const voice = this.lang.language === this.lang.defaultLanguage ? this.lang.defaultVoice : false;

		this.voice.speak(message, this.mod.settings.rate, voice, e => {
