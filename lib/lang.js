	...
	applyDungeons() {
		...
		Object.keys(this.mod.settings.dungeons).forEach(zoneId => {
			...
			if(defaultStrings[zoneId])
				self.mod.settings.dungeons[zoneId].name = defaultStrings[zoneId][self.language] || defaultStrings[zoneId][self.defaultLanguage];

			// Read custom strings
			if(strings[zoneId] && strings[zoneId][self.language])
				self.mod.settings.dungeons[zoneId].name = strings[zoneId][self.language];

			// Delete zones without names/translation from config
			if (!self.mod.settings.dungeons[zoneId].name)
				delete self.mod.settings.dungeons[zoneId];
		});
	}
}
