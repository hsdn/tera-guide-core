...
class Lang {
	...
	init() {
		...
		this.getSystemCulture((err, data) => {
			...
			const systemCulture = data.trim();
			const systemLang = systemCulture.split("-")[0];

			this.voice.getInstalledVoices((e, voices) => {
				if (e) return;

				if (systemLang === defaultLanguage)
					// If the systemLang is the same as defaultLanguage, set the voice of systemLang
					voices.forEach(val => {
						if (val.culture === systemCulture && val.gender === "Female")
