...
class Powershell extends EventEmitter {
	...
	start() {
		return new Promise((resolve, reject) => {
			...
			this.errorCb = error => reject(error);
			this.removeAllListeners();

			this.proc = spawn(wrapperCommand);

			if(!this.proc.pid)
				this.emit("end", new PS_PROC_ERROR);
			...
			this.proc.once("close", code => {
				this.emit("error", new Error(`Wrapper process ${this.proc.pid} exited with code ${code}`));
			});

			this.proc.stdin.on("error", error => { this.emit("end", new PS_PROC_ERROR(error)); });
			this.proc.stdin.on("close", error => { this.emit("end", new PS_PROC_ERROR(error)); });
			...
			this.on("error", error => { this.errorCb(error); });
			this.on("end", error => {
				this.stop();
