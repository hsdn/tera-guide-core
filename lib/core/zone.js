...
class Zone {
	...
	async init() {
		...
		(await readdir(path.resolve(this.__deps.mod.info.path, "guides")))
			.forEach(file => {
	...
	__migrateSettings() {
		if (typeof this.__deps.mod.settings.dungeons !== "object" || Array.isArray(this.__deps.mod.settings.dungeons))
