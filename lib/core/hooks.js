...
class Hooks {
	...
	constructor(deps, guide) {
		...
		this.config = {
			"S_ACTION_STAGE": { "version": 9, "order": 15, "keys": ["s"], "debug": "skill" },
			"S_ABNORMALITY_BEGIN": { "version": 4, "order": 15, "keys": ["am", "ae", "ab"], "debug": "abnormal" },
			"S_ABNORMALITY_REFRESH": { "version": 2, "order": 15, "keys": ["am", "ae", "ab"], "debug": "abnormal" },
			"S_ABNORMALITY_END": { "version": 1, "order": 15, "keys": ["ar", "ad"], "debug": "abnormal" },
			"S_BOSS_GAGE_INFO": { "version": 3, "order": 0, "keys": ["h"], "debug": "hp" },
			"S_SPAWN_NPC": { "version": 11, "order": 0, "keys": ["ns"], "debug": "spawn" },
			"S_DESPAWN_NPC": { "version": 3, "order": -100, "keys": ["nd"], "debug": "spawn" },
			"S_DUNGEON_EVENT_MESSAGE": { "version": 2, "order": 0, "keys": ["dm"], "debug": "dm" },
			"S_QUEST_BALLOON": { "version": 1, "order": 0, "keys": ["qb"], "debug": "qb" },
		};
	}
	...
	load(keys, debug_enabled = false) {
		...
		Object.keys(this.config).forEach(name => {
			...
			entry.keys.forEach(key => {
				...
				let debug = self.mod.settings.debug.all || self.mod.settings.debug[entry.debug];
	...
	s_action_stage(event, debug_enabled) {
		...
		if (ent) {
			...
			this.guide.handleEvent({ ...ent, ...event }, skillid, "s", {
				"enabled": debug_enabled || (ent.templateId % 1 === 0 ? this.mod.settings.debug.boss : false),
				"name": "Skill",
				"color": cy
