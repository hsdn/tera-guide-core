...
class Hooks {
		...
		Object.keys(self.config).forEach(name => {
			...
			entry.forEach(key => {
				...
				try {
					...
					self.handlers.send.debug(debug_enabled || self.mod.settings.debug.all, `Add hook: ${name} [${entry.toString()}]`);
				}
				catch (e) {
	...
	unload(debug_enabled = false) {
		...
		Object.keys(this.guide.obj.hooks).forEach(name => {
			...
			self.handlers.send.debug(debug_enabled || self.mod.settings.debug.all, `Remove hook: ${name} [${entry.keys.toString()}]`);

			delete self.guide.obj.hooks[name];
		});
	}
