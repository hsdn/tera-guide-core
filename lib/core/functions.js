...
class Functions {
	...
	constructor(deps) {
		...
		if (this.params.colors.general) Object.assign(global, this.params.colors.general);
		if (this.params.colors.gui) Object.assign(global, this.params.colors.gui);
	}

	destructor() {
		Object.keys(this.params.colors.general).forEach(key => {
			delete global[key];
		});

		Object.keys(this.params.colors.gui).forEach(key => {
			delete global[key];
		});
	}
}
