...
class Functions {
	...
	constructor(deps) {
		this.__deps = deps;
		this.__optionEffects = {};

		// Hooks skill poslishing effects
		this.__deps.mod.hook("S_RP_SKILL_POLISHING_LIST", 1, event => {
			this.__optionEffects = {};

			event.optionEffects.forEach(effect => {
				if (effect.active === true) {
					this.__optionEffects[effect.group] = effect.id;
				}
			});
		});
	}
	...
	classPositionCheck(class_position) {
		...
		switch (class_position) {
			case "tank":
				...
				// if it's a berserker with Intimidation abnormality
				if (player.job === 3 && effect.hasAbnormality(401400))
					return true;

				// if it's a brawler with tank poslishing
				if (player.job === 10 && this.__optionEffects[1104] === 17111802)
					return true;
				break;

			case "dps":
				// If it's a warrior without dstance abnormality
				if (player.job === 0) {
					// Loop thru tank abnormalities
					for (const id of [100200, 100201])
				...
				// if it's a berserker without Intimidation abnormality
				if (player.job === 3 && !effect.hasAbnormality(401400))
					return true;

				// if it's a brawler without tank poslishing
				if (player.job === 10 && this.__optionEffects[1104] !== 17111802)
					return true;

				// if it's a dps return true
				if ([2, 4, 5, 8, 9, 11, 12].includes(player.job)) return true;
				break;
