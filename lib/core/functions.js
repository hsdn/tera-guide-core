...
class Functions {
	...
	constructor(deps) {
		...
		Object.assign(global, this.__params.colors.general);
		Object.assign(global, this.__params.colors.gui);
	}
	...
	classPositionCheck(class_position) {
		const { player, effect } = this.__mod.require.library;
		...
		switch (class_position) {
			...
			default:
				this.__mod.warn(`Failed to find class position: ${class_position}`);
		}
	...
	destructor() {
		Object.keys(this.__params.colors.general).forEach(key => { delete global[key]; });
		Object.keys(this.__params.colors.gui).forEach(key => { delete global[key]; });
	}
}
