...
module.exports.callback = (deps, guide, event) => {
	...
	if (event.source && player.playersInParty.includes(event.source.toString())) return;
	...
	if (player.isMe(event.target)) {
		...
		if (source_ent)
			return guide.handleEvent(["am", source_ent.huntingZoneId, source_ent.templateId, event.id], source_ent, module.exports.debug);
	} else {
