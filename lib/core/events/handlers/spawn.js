...
const { applyDistance, Spawn } = require("../../../spawn");
...
module.exports.spawn = (deps, data, event, ent) => {
	...
	if (!deps.mod.settings.spawnObject || !deps.zone.settings.spawnObject) return;
	...
	loc = applyDistance(loc, event.distance || 0, event.angle || 0);
