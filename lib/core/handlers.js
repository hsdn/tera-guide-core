...
class Handlers {
	...
	spawn(event = {}, ent = false, speed = 1.0) {
		...
		// Despawn a spawned item
		this.despawn({
			id: item_unique_id,
			sub_type: sub_type,
			delay: parseInt(event["sub_delay"])
		}, ent, speed);
	}
	spawn_func(...args) {
		this.spawn(...args);
	...
	despawn_all(event = {}, ent = false, speed = 1.0) {
		...
		const callback = () => {
			...
			// Final list cleanup
			self.guide.obj.spawned_items = {};
		};

		// Create timer for specified delay
		if (delay > 0)
			this.mod.setTimeout(callback, delay / speed);
		else
			callback();
	}
	...
	marker(event = {}, ent = false, speed = 1.0, add = true) {
		// Ignore if streamer mode is enabled
		if (this.mod.settings.stream) return;
		...
		if (!event["id"]) return this.mod.error("Add/remove marker handler needs a gameId (id)");
		...
		const callback = (event, add) => {
			if (add) {
				...
				// Add the marker
				self.guide.obj.marker_ids[event["id"]] = event["color"];
			} else {
				// Remove the marker
			...
			for (const [gameId, color] of Object.entries(self.guide.obj.marker_ids)) {
				targets.push({
					target: gameId,
					color: color 
				});
		...
		// Create timer for specified delay
		if (delay > 0)
			this.mod.setTimeout(callback, delay / speed, event, add);
		else
			callback(event, add);
		...
		if (sub_delay > 0) {
			...
			this.mod.setTimeout(() => {
				this.marker_remove({ id: event["id"] }, ent, speed);
			}, sub_delay / speed);
		}
	}
	marker_add(...args) {
	...
	marker_remove(event = {}, ent = false, speed = 1.0) {
		this.marker(event, ent, speed, false);
	}
