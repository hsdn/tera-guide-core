...
class Handlers {
	...
	spawn(event = {}, ent = false, speed = 1.0) {
		...
		if (event["func"]) {
			...
			try {
				this.delayEvent((self, event, ent) => {
					...
					const instance = new Spawn(ent, self.mod, self);
		...
		this.delayEvent((self, event, ent, sub_type, item_unique_id) => {
		...
		}, delay, speed, event, ent, sub_type, item_unique_id);
	...
	delayEvent(callback, delay, speed, ...args) {
		if (delay > 0)
			this.mod.setTimeout(callback, delay / speed, this, ...args);
		else
			callback(this, ...args);
	}
