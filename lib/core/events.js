...
module.exports = (mod, guide, lang, dispatch, params) => {
	...
	mod.game.on("enter_game", () => {
		async function initConfiguration() {
			...
			try {
				strings = require(path.resolve(mod.info.path, "lang", "dungeons"));
			} catch (e) { mod.error(e); }
			...
			for (const file of guideFiles) {
				...
				if(defaultStrings[zoneId])
					mod.settings.dungeons[zoneId].name = defaultStrings[zoneId][lang.language] || defaultStrings[zoneId][lang.defaultLanguage];

				if(strings[zoneId])
					mod.settings.dungeons[zoneId].name = strings[zoneId][lang.language] || strings[zoneId][lang.defaultLanguage] || defaultStrings[zoneId][lang.defaultLanguage];
			}
			...
			try {
				...
				for (const res of resTwo){
					...
					if (!mod.settings.dungeons[id] || mod.settings.dungeons[id]["name"]) continue;
				...
				for (const file of guideFiles) {
					if (!file.endsWith(".js")) continue;
					const zoneId = file.split(".")[0];
					if (!mod.settings.dungeons[zoneId] || mod.settings.dungeons[zoneId]["name"]) continue;
