		...
		mod.command.add(params.command, {
			...
			"event": (arg1, arg2) => {
				...
				if (["status", "s"].includes(arg1)) {
					// Guide status
					if (guide.obj.loaded) {
						this.__mod.command.message(`${cw}Guide ID: ${cy}${guide.obj.id}`);
						this.__mod.command.message(`${cw}Guide name: ${cy}${guide.obj.name || "not defined"}`);
						// eslint-disable-next-line no-nested-ternary
						this.__mod.command.message(`${cw}Guide type: ${cy}${guide.obj.type === SP ? "SP" : (guide.obj.type === ES ? "ES" : "standard")}`);
					} else
						this.__mod.command.message(`${cy}Guide not loaded.`);
