		...
		mod.command.add(params.command, {
			...
			debug(arg1) {
				...
				if (arg1 === "status") {
					Object.keys(mod.settings.debug).forEach(key => {
						mod.command.message(`debug(${key}): ${mod.settings.debug[key] ? "enabled" : "disabled"}.`);
					});
					return;
			...
			event(arg1, arg2) {
				if (!mod.settings.enabled)
					return mod.command.message("Module is disabled");
				...
				if (arg1 === "trigger") {
					handlers.start_events(guide.obj.context[arg2], player);
				} else {
					try {
