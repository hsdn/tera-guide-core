...
class Guide extends EventEmitter {
	...
	addEvents(debug_enabled = false) {
		...
		if (debug_enabled) {
			this.handlers.send.debug(true, "Added events:");
			this.eventNames().forEach(key => {
				this.handlers.send.debug(true, `${key} (${this.listenerCount(key)})`);
			});
		}
	}
