...
class Guide {
	...
	load(zone, debug_enabled = false) {
		...
		this.handlers.text({
			...
			"message": `${this.lang.strings.enterdg}: ${cr}${this.obj.name} ${cw}[${this.obj.id}]`
		});
		this.handlers.text({
			"sub_type": "CGMSG",
			"delay": debug_enabled ? 0 : 5000,
			"message": `${this.lang.strings.helpheader}\n` +
				`${this.lang.strings.stream}: ${this.mod.settings.stream ? this.lang.strings.enabled : this.lang.strings.disabled}\n` +
				`${this.lang.strings.gNotice}: ${this.mod.settings.gNotice ? this.lang.strings.enabled : this.lang.strings.disabled}\n` +
				`${this.lang.strings.speaks}: ${this.mod.settings.speaks ? this.lang.strings.enabled : this.lang.strings.disabled}`
		});

		// Set this.obj as loaded
