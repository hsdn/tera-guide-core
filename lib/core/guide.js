...
class Guide {
	...
	async init() {
		...
		guideFiles.forEach(file => {
			...
			if(defaultStrings[zoneId])
				self.mod.settings.dungeons[zoneId].name = defaultStrings[zoneId][self.lang.language] || defaultStrings[zoneId][self.lang.defaultLanguage];

			if(strings[zoneId])
				self.mod.settings.dungeons[zoneId].name = strings[zoneId][self.lang.language] || strings[zoneId][self.lang.defaultLanguage] || defaultStrings[zoneId][self.lang.defaultLanguage];
		});
	...
	// Handle events
	handleEvent(ent, evtid, prefix, debug, speed = 1.0, stage = false) {
		// Get key string
		let key = `${prefix}-${ent["huntingZoneId"]}-${ent["templateId"]}`;

		if (evtid !== false) key += `-${evtid}`;
