...
class Guide {
	...
	load(zone, debug_enabled = false) {
		...
		this.handlers.text({
			...
			"message": `${this.lang.strings.helpheader}\n` +
				`${this.lang.strings.stream}: ${cr}${this.mod.settings.stream ? this.lang.strings.enabled : this.lang.strings.disabled}\n` +
				`${this.lang.strings.speaks}: ${cr}${this.mod.settings.speaks ? this.lang.strings.enabled : this.lang.strings.disabled}`
		});
	...
	unload() {
		...
		// Force remove all markers from ids and clear guide.marker_ids
