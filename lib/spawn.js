...
class Spawn {
	...
	item(item, angle, distance, delay, duration) {
		this.object("item", false, item, 0, 0, Math.PI * angle / 180, distance, delay, duration, null);
	}
	...
	marker(target, angle, distance, delay, duration, highlight, label) {
		...
		this.object("build_object", target, 1, 0, 0, angle, distance, delay, duration, label || ["SAFE SPOT", "SAFE"]);
		...
		if (highlight) {
			...
			this.object("item", target, item, 0, 0, angle, distance, delay, duration, null);
		}
	}
	...
	point(item, angle, distance, delay, duration) {
		this.object("collection", false, item, 0, 0, Math.PI * angle / 180, distance, delay, duration, null);
	}
	...
	vector(item, offsetAngle, offsetDistance, angle, length, delay, duration) {
		...
		for (let radius = 50; radius <= length; radius += 50) {
			this.object("collection", false, item, offsetAngle, offsetDistance, angle, radius, delay, duration, null);
		}
	}
	...
	circle(target, item, offsetAngle, offsetDistance, interval, radius, delay, duration) {
		for (let angle = -Math.PI; angle <= Math.PI; angle += Math.PI * interval / 180) {
			this.object("collection", target, item, offsetAngle, offsetDistance, angle, radius, delay, duration, null);
		}
	}
	 ...
	 * @param {boolean} target Using "dest" instead of "loc" when defining an anchor point
	 * @param {(number|string)} item Item identifier of the spawned object items
	 * @param {number} offsetAngle Offset angle relative to binding point (e.g. boss)
	 * @param {number} offsetDistance Offset distance relative to binding point (1 meter = 25 units)
	 ...
	 * @param {number} delay Object spawn time delay
	 * @param {number} duration The lifetime of the object (before its despawn)
	 * @param {string[]} label Array of text label (used only with "build_object" type)
	 * @memberof Spawn
	 */
	object(type, target, item, offsetAngle, offsetDistance, angle, distance, delay, duration, label) {
		...
		this.__handlers.delay(() => {
			// The "dest" is a entity object of the boss skill taken from S_ACTION_STAGE.
			// Its needed on some mechanics for more accurate binding to NPC in action (e.g. boss).
			// Best practice is to use "dest" for items attached to the boss action stage,
			// and use "loc" for items when the boss is standing (AoEs, donuts, waves, etc.)

			// Use "dest" instead of "loc" if it's specified and target is true
			if (target && this.__entity.dest !== undefined)
				shield_loc = this.__entity.dest.clone();
			// Use "loc" if it's specified and target is false
			else if (this.__entity.loc !== undefined)
				shield_loc = this.__entity.loc.clone();
			else
				return;

			// Set value of loc.w for the binding point
			shield_loc.w = this.__entity["loc"].w;
